function m$B2(t,a){var l=this;l.ind=t,l.rowdata=a,l.ellipses=[],l.label=$("<div class='bblabel'></div>"),l.html=$("<div class='bbbox'></div>"),l.ind.container.append(l.html),l.paper=new Raphael(l.html[0]),l.html.append(l.label),l.invalidate=-1,l.validateData.call(l)}function m$Bl(t){this.parent=t,this.mv=null,this.colorset=[.23,.4,.3],this.initVariables()}m$B2.prototype={validateData:function(){var t,a,l=this,e=l.ellipses,i=l.ind.colorset,n=l.rowdata;for(t=0;t<e.length;t++)e[t].html.remove();for(e=[],l.label.text(n.name),t=0;t<n.value.length;t++)(a={}).color=i[t%i.length],e.push(a);l.ellipses=e,l.validateDisplay()},validateDisplay:function(){var t=this;-1<t.invalidate&&clearTimeout(t.invalidate),setTimeout(function(){t.updateDisplay.call(t)},100)},updateDisplay:function(){var t,a,l,e,i,n,h,o,s,r,m,c,p=this,u=p.ind,v=p.html,b=IG$.j$ext._w(v),d=IG$.j$ext._h(v),f=p.rowdata,x=p.ellipses,$=u.mcalc.max,g=null==u.mv?u.mcalc.min:u.mv>u.mcalc.min?u.mv:u.mcalc.min,y=p.label;if(0<b&&0<d){if((s=p.paper).clear(),s.setSize(b,d),x&&0<x.length){for(e=Math.min(b,d),a=$==g?$:$-g,o=0;o<x.length;o++)h=0==o?f.value[o]:h+f.value[o];for(n=(h-g)/a*(.8*e)+.1*e,o=0;o<x.length;o++)i=0==o?f.value[o]:i+f.value[o],l=Math.sqrt(n*n*i/h),(t=x[o]).x=b/2,t.y=d/2,t.radius=l/2;for(o=x.length-1;0<=o;o--)r=(t=x[o]).x,m=t.y,c=t.radius,hue=t.color,t.circle=s.set(s.ellipse(r,m+c-c/5,0,0).attr({fill:"rhsb("+hue+", 1, .25)-hsb("+hue+", 1, .25)",stroke:"none",opacity:0}).animate({rx:c,ry:c/2},400,"<"),s.ellipse(r,m,0,0).attr({fill:"r(.5,.9)hsb("+hue+", 1, .75)-hsb("+hue+", .5, .25)",stroke:"none"}).animate({rx:c,ry:c},400,"<"),s.ellipse(r,m,0,0).attr({stroke:"none",fill:"r(.5,.1)#ccc-#ccc",opacity:0}).animate({rx:c-c/5,ry:c-c/20},400,"<"))}p.label.css({top:d-y.height()-4,left:(b-IG$.j$ext._w(p.label))/2})}}},m$Bl.prototype={initVariables:function(){var t=this;t.ex=[],t.mx=[],t.bubbles=[],t.mcalc={},t.container=$("<div class='bbmain'></div>").appendTo(t.parent)},initControl:function(){var t,a=this.bubbles;for(t=0;t<a.length;t++)a[t].html.remove();a=[]},loadData:function(t){var a,l,e,i,n,h,o,s,r=this,m=r.ea,c=r.ma,p=r.mcalc;if(t&&0<t.length&&m&&0<m.length&&c&&0<c.length){for(a=0;a<c.length;a++)c[a].max=0,c[a].min=0;for(a=0;a<t.length;a++){for("col_"&a,i="",h=[],l=0;l<m.length;l++)e=m[l].i,i=0==l?t[a][e]:i+" "+t[a][e];for(l=0;l<c.length;l++)e=c[l].i,n=Number(t[a][e]),0==a?(c[l].min=n,c[l].max=n):(c[l].min=Math.min(c[l].min,n),c[l].max=Math.max(c[l].max,n)),o=0==l?n:o+n,h.push(n);0==a?(p.min=o,p.max=o):(p.min=Math.min(p.min,o),p.max=Math.max(p.max,o)),s=new m$B2(r,{name:i,value:h}),r.bubbles.push(s)}r.validateControl()}},validateControl:function(){var t,a,l,e,i,n=$(this.parent),h=IG$.j$ext._w(n),o=IG$.j$ext._h(n),s=this.bubbles;if(0<h&&0<o&&s&&0<s.length)for(i=0,l=h/s.length,e=o,a=0;a<s.length;a++)(t=s[a]).html.css({left:i,top:0,position:"absolute"}),IG$.j$ext._w(t.html,l),IG$.j$ext._h(t.html,e),t.validateDisplay.call(t),i+=l}};
