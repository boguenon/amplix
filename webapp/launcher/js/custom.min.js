...
IG$.__chartoption.charttype=IG$.__chartoption.charttype||[],IG$.__chartoption.charttype.push({label:"Naver Map",charttype:"navermap",subtype:"navermap",img:"map_naver",grp:"scientific"}),IG$.__chartoption.chartext.navermap=function(a){},IG$.__chartoption.chartext.navermap.prototype={drawChart:function(a,t){var n=this;IG$.__chartoption.chartext.navermap_main?(n.dmain&&(n.dmain.dispose(),n.dmain=null),n.dmain=new IG$.__chartoption.chartext.navermap_main(a),n.dmain.drawChart.call(n.dmain,a,t)):IG$.__chartoption.chartext.navermap._loaded||IG$.getScriptCache(["./custom/custom.map.naver.worker.js"],new IG$.callBackObj(this,(function(){IG$.__chartoption.chartext.navermap._loaded=1;var r=new IG$.__chartoption.chartext.navermap_main(a);n.dmain=r,r.drawChart.call(r,a,t)})))},updatedisplay:function(a,t,n){this.dmain&&this.dmain.updatedisplay.call(this.dmain,a,t,n)}};
...
IG$.__chartoption.charttype.push({label:"TimeLine",charttype:"timeline",subtype:"timeline",img:"timeline",grp:"cartesian"}),IG$.__chartoption.chartext.timeline=function(t){this.owner=t},IG$.__chartoption.chartext.timeline.prototype={drawTimeline:function(t,e){var a,r,n=$(t.container),s=(t._ILb,t.cop),i=(s.usedualaxis,s.dualaxisitem,e.data);this.owner=t,this.container=n,n.empty(),(r=$("<div class='igc-tg-gr'></div>").appendTo(n)).width(n.width()),r.height(n.height());var l=30,o=0,c=40,d=120,p=950-d-o,h=220-l-c,m=d3.scale.linear().domain([0,152100]).range([0,p]),u=d3.format("02d"),f=function(t){return Math.round(t/1e3)+" sec"};formatTimecode=function(t){return u(t/6e4|0)+":"+u((t/1e3|0)%60)};var y=[],x={slug:"silver_linings_playbook",name:"Silver Linings Playbook",key_frame:93,youtube:"tFvMk983Il8",intro:"��Silver Linings Playbook�� follows the standard model for trailers, according to <a href='http://www.billwoolery.com/'>Bill Woolery</a>, a trailer specialist in Los Angeles who once worked on trailers for movies like ��The Usual Suspects�� and ��E.T. the Extra-Terrestrial.�� While introducing the movie��s story and its characters, the trailer largely follows the order of the film itself.",annotations:[{trailer_ms:6500,width:280,offset:-40,movie_ms:6890055,text:"The trailer��s opening shot ??an image of the family��s home ??appears near the end of the film, but there are similar shots near the beginning of the movie."},{trailer_ms:105081,width:210,offset:-140,movie_ms:-1,text:"A handful of very short shots are never seen in the film, although most are shown from alternate camera angles."},{trailer_ms:129600,width:150,offset:-40,movie_ms:6187879,text:"Shots that accompany the main actors�� names are also shown out of order."}],fps:29.97002997002997,movie_length_frames:175620,movie_length_ms:7324826,search_bounds:[0,167083],bounds_ms:[15812,6967778],trailer:{thumb_width:266,trailer_length_frames:3793,trailer_length_ms:151235,shots:[]}};for(a=e.rowfix;a<i.length;a++)x.trailer.shots.push({start_ms:100*a,match_ms:Number(i[a][1].code)});y.push(x);var g,v,_,b=d3.select(r[0]).selectAll(".movie").data(y).enter().append("div").attr("class","movie");function w(t){d3.event.metaKey||d3.event.altKey||(d3.select(".lightbox").style("display","block").select("iframe").attr("src","http://www.youtube.com/embed/"+t.youtube+"?rel=0&autoplay=1"),d3.event.preventDefault())}function k(t){d3.select(".lightbox").style("display","none").select("iframe").attr("src","about:blank")}b.append("h3").text((function(t){return t.name})),b.append("p").classed("intro",!0).html((function(t){return t.intro})),b.each((function(t,e){var a=d3.select(this),r=t.movie_length_ms;sectionDuration=r/3,trailerData=t.trailer,time_ms=trailerData.trailer_length_ms,shotsData=trailerData.reviewed_shots||trailerData.shots,shotsData.slice().reverse().forEach((function(t){t.length_ms=time_ms-(time_ms=t.start_ms),(t.missing||t.suppress)&&(t.match_ms=-1)}));var n=[{label:"Beginning of film",ms:sectionDuration},{label:"Middle",ms:2*sectionDuration},{label:"End",ms:3*sectionDuration}],s=d3.scale.linear().domain([-1,0,r]).range([h+12.5,0,h]),i=d3.svg.axis().scale(m).tickSize(5).tickSize(15).tickValues([0,3e4,6e4,9e4,12e4]).orient("top").tickSubdivide(3).tickFormat(f),u=(d3.svg.line().x((function(t){return s(t.x)})).y((function(t){return t.y})).interpolate("cardinal"),d3.svg.line().x((function(t){return m(t.start_ms)})).y((function(t){return s(t.match_ms)})).interpolate("step-after")),y=d3.scale.threshold().domain([0,sectionDuration,2*sectionDuration]).range(["#ccc","#fd9226","#fa4f43","#a20f79"]),x=t.trailer.thumb_width,g=a.append("a").classed("thumbnail-frame",!0).style("left",p+d+o-12-x+"px").attr("href","http://www.youtube.com/watch?v="+t.youtube).on("click",w);g.append("div").classed("thumbnail-crop",!0).style("width",x-2+"px").style("height","108px").append("img").classed("thumbnail-image",!0).attr("src","http://graphics8.nytimes.com/newsgraphics/2013/01/04/movie-marker/b1a9c2fc1689fd89ecff4a7d2be5e5757716e344/thumbnails/"+t.slug+".jpg"),g.append("div").classed("play",!0).append("div").classed("label",!0).text("Watch");var v=a.append("div").classed("stage",!0),_=v.append("svg").attr("width",p+d+o).attr("height",h+l+c).append("g").attr("transform","translate("+d+","+l+")"),b=_.selectAll(".annotation-pointer").data((function(t){return t.annotations})).enter().append("g").classed("annotation-pointer",!0).attr("transform",(function(t){return"translate("+m(t.trailer_ms)+",0)"}));b.append("line").attr("y1",h+25+10).attr("y2",(function(t){return s(t.movie_ms)+8})),b.append("path").attr("x",-1.5).attr("y",(function(t){return s(t.movie_ms)+8})).attr("d",(function(t){var e=s(t.movie_ms)+8;return"M -3 "+(e+3)+" L 0 "+e+" L 3 "+(e+3)})).attr("width",3).attr("height",3),v.append("div").classed("annotations",!0).style("left",d+"px").selectAll(".annotation").data((function(t){return t.annotations})).enter().append("div").classed("annotation",!0).text((function(t){return t.text})).style("width",(function(t){return t.width+"px"})).style("left",(function(t){return m(t.trailer_ms)+t.offset+"px"})).style("top",h+l+25+10+"px"),_.append("path").datum(shotsData.filter((function(t){return!t.suppress&&!t.missing}))).attr("class","scatterline").attr("d",u),_.append("g").attr("class","x axis").call(i).append("text").attr("x",7).attr("y",-22).text("Start of trailer →");var k=_.selectAll(".x.axis g:first-of-type text").select((function(){return this.parentNode.appendChild(this.cloneNode(!0))})).datum(null).style("display","none"),D=_.append("g").attr("class","y axis");D.append("line").classed("baseline",!0).attr("x1",-d).attr("y1",s(0)).attr("x2",p+o).attr("y2",s(0)),D.append("line").classed("separator",!0).attr("y1",-l).attr("y2",h+25);var A=D.selectAll(".section-marker").data(n).enter().append("g").classed("section-marker",!0).attr("transform",(function(t){return"translate(0,"+s(t.ms)+")"}));A.append("rect").attr("x",m(0)).attr("y",-h/3).attr("width",p+o).attr("height",h/3).style("fill",(function(t){return y(t.ms-1)})),A.append("line").classed("section-line",!0).attr("x1",-d).attr("x2",p+o),A.append("text").classed("label",!0).attr("y",-h/6).attr("dy",".35em").attr("x",-15).style("fill",(function(t){return y(t.ms-1)})).text((function(t){return t.label}));var M=D.append("g").attr("class","missing-section").attr("transform","translate(0,"+h+")");M.append("text").text("Not in film").attr("y",12.5).attr("dy","0.4em").attr("x",-15),M.append("line").classed("section-line",!0).attr("y1",25).attr("y2",25).attr("x1",-d).attr("x2",p+o);var T=_.selectAll(".cut").data(shotsData).enter().append("g").attr("class","cut").on("mouseover",L).on("mouseout",S);function L(e,r){k.classed("frame-label",!0).style("display",null).text(formatTimecode(e.start_ms)).attr("x",Math.max(6,Math.min(p-k.node().getComputedTextLength()-6,m(e.start_ms)))),_.selectAll(".x.axis text").style("opacity",(function(t){return Math.abs(e.start_ms-t)<1e4?0:null})),a.select(".thumbnail-image").style("left",-Math.round(t.trailer.thumb_width*(r%10)+2)+"px").style("top",-Math.round(110*Math.floor(r/10)+2)+"px"),a.select(".play").style("opacity",0)}function S(){_.selectAll(".x.axis text").style("opacity",null),k.style("display","none"),a.select(".play").style("opacity",.5)}T.filter((function(t){return!0/*!d.suppress*/})).append("rect").classed("bar",!0).attr("data-id",(function(t,e){return e})).attr("width",(function(t){return Math.max(m(t.length_ms)-1,1)})).attr("height",8).attr("rx",1).attr("y",(function(t){return s(t.match_ms||0)-4})).attr("x",(function(t){return m(t.start_ms)})).attr("fill",(function(t){return y(t.match_ms||0)})),T.append("rect").classed("bar-hover",!0).attr("width",(function(t){return m(t.length_ms)})).attr("height",h+25-1).attr("y",1).attr("x",(function(t){return m(t.start_ms)})),L(t,t.key_frame),S()})),d3.selectAll(".x.axis g text").style("text-anchor","start").attr("x",7).attr("y",-7),d3.selectAll(".x.axis .tick.minor").attr("y2",-4),g=720,v=405,(_=d3.select("#g-graphic").append("div").classed("lightbox",!0).style("display","none").on("click",k).append("div").style("position","absolute").style("left","50%").style("top","50%").append("div").style("width","760px").style("height","445px").style("position","absolute").style("left","-380px").style("top","-222.5px").style("background-color","black")).append("div").attr("id","trailersVideoPlayerContainer").style("width","720px").style("height","405px").append("iframe").attr("width",g).attr("height",v).attr("frameborder",0).property("allowfullscreen",!0).style("position","absolute").style("left","20px").style("top","20px"),_.append("a").attr("class","g-close-button").text("Close")},drawChart:function(t,e){this.drawTimeline(t,e)},updatedisplay:function(t,e,a){}};
