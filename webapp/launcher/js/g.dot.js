...
(function () {
    ...
    var colorValue = function (value, total, s, b) {
        ...
        var ch = Math.min((1 - value / total) * .4, 1),
        	cs = s || .75,
        	cb = b || .75,
        	c = HSVtoRGB(ch, cs, cb);
    ...
    function Dotchart(paper, x, y, width, height, valuesx, valuesy, size, opts) {
        ...
        function drawAxis(ax) {
            ...
            +ax[2] && (ax[2] = chartinst.axis(x + gutter, y + height - 40, width - 2 * gutter, minx, maxx, opts.axisxstep || Math.floor((width - 2 * gutter) / 20), 0, opts.axisxlabels || null, opts.axisxtype || "t", null, paper));
            +ax[3] && (ax[3] = chartinst.axis(x + gutter - maxR, y + height - 60, height - 2 * 40, miny, maxy, opts.axisystep || Math.floor((height - 2 * gutter) / 20), 1, opts.axisylabels || null, opts.axisytype || "t", null, paper));
        }
...
//        }
        ...
        var kx = (width - gutter * 2) / ((maxx - minx) || 1),
	        ky = (height - 80) / ((maxy - miny) || 1);
	
	    for (var i = 0, ii = valuesy.length; i < ii; i++) {
	        var sym = paper.raphael.is(symbol, "array") ? symbol[i] : symbol,
	            X = x + gutter + (valuesx[i] - minx) * kx,
	            Y = y + height - 60 - (valuesy[i] - miny) * ky;
